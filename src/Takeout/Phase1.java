...
public class Phase1 {
    ...
    public static void main(String[] args) {
        ...
        do {
            ...
            //Start Ordering Food
            System.out.println("AVAILABLE TAKEOUT MENU");
            Phase1.displayMainMenu();
            ...
            switch (mealType) {
                ...
                case "1":
                    Phase1.displayItalianFoodMenu();
                    int[] feedItalianFood = {5, 7, 4};
                    double[] priceItalian = {17.99, 15.99, 12.99};
                    int opt = Phase1.foodChoice(feedItalianFood, priceItalian);
                    Phase1.finalPrice(feedItalianFood, priceItalian, opt, numberOfPeople);
                    isWrongMenuChoice = false;
                    break;

                case "2":
                    Phase1.displayChineseFoodMenu();
                    int[] feedChineseFood = {7, 7, 5};
                    double[] priceChinese = {18.99, 18.99, 10.99};
                    opt = Phase1.foodChoice(feedChineseFood, priceChinese);
                    Phase1.finalPrice(feedChineseFood, priceChinese, opt, numberOfPeople);
                    isWrongMenuChoice = false;
                    break;

                case "3":
                    Phase1.displayAmericanFoodMenu();
                    int[] feedAmericanFood = {8, 5, 10};
                    double[] priceAmerican = {21.99, 22.99, 26.99};
                    opt = Phase1.foodChoice(feedAmericanFood, priceAmerican);
                    Phase1.finalPrice(feedAmericanFood, priceAmerican, opt, numberOfPeople);
                    isWrongMenuChoice = false;
                    break;
                default:
...
//phase2
    ...
    public static void finalPrice(int[] feeds, double[] prices, int choice, int guestCount) {
        int trays = Phase1.determineTrays(guestCount, feeds[choice-1]);
        double subTotal = Phase1.getSubtotal(prices[choice - 1], trays);
        double tax = Phase1.getTax(subTotal, .07);
        double tip = Phase1.getTip(subTotal, .15);
        double grandTotal = Phase1.getGrandTotal(subTotal, tax, tip);
        ...
        System.out.printf("Price per person: $ %.2f\n", Phase1.pricePerPerson(grandTotal, guestCount));
        System.out.println("Left-over servings for the delivery person: " + Phase1.determineLeftOvers(feeds[choice - 1], trays, guestCount));
    }
